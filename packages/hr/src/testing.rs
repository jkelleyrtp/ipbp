...
fn analyze_imports() {
    ...
    dbg!(old_bj
        ...
        .unwrap()
        .into_iter()
        .map(|f| std::str::from_utf8(f.name()).unwrap())
        .collect::<Vec<_>>());
}

#[test]
fn write_new_with_manually_massaged_relocations() {
    let contents = include_bytes!("../../../saved/arrow/jx7vacigf9h88k6.o");
    let old_bj = object::read::File::parse(contents as &[u8]).unwrap();

    let mut new_bj =
        object::write::Object::new(old_bj.format(), old_bj.architecture(), old_bj.endianness());

    for section in old_bj.sections() {}
}
            ...
            match target_sym {
                RelocationTarget::Symbol(sym) => {
                        ...
                        .any(|f| f.name() == sym.name_bytes().unwrap())
                    {
                        ...
                        // These we just want the linker to assume we've done the right thing...
                        ...
                        let reloc_type = match last_reloc.as_ref() {
                            Some(last) => {
                        ...
                        println!(
                            "[{offset:05} [{reloc_type}] reloc] {:08x?} @ {:?} - {:?}",
                            sym.address(),
                            sym.name().unwrap(),
